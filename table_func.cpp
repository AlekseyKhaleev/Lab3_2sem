...
int CreateTable(char *argv[], std::fstream &table_out, int rows_request) {
    ...
    *       начиная с индекса row_number и заканчивая i-1, так как i соответствует не инициализированному аргументу.
    *       - с помощью метода ParkUpdate[i].set() осуществляется ввод значений очередной структуры
    *   (строки) в последней строке таблицы.
    *       - инициализированная структура записывается в двоичный файл
    *       - переменная row_number, соответствующая номеру первой значащей строки, отображаемой в данный момент
    ...
    *******************************************************************************************************************/
    ...
    int row_number = 1;                            // номер первой значащей строки, отображаемой в данный момент
    try {
        ...
        auto ParkUpdate = new PC[rows_request];    // выделение памяти
        for (int i = 0; i < rows_request; i++) {
            if (i < MAX_ROWS)
                /* Если не достигнута последняя строка в отображаемой таблице */
            {
                ParkUpdate[i].set(i+1, i); // ввод данных в соответствующих ячейках
                table_out.write((char *) &ParkUpdate[i], sizeof(struct PC)); // запись структуры в файл
            } else
                /* Если достигнута последняя строка в отображаемой таблице */
            {
                ...
                table_out.write((char *) &ParkUpdate[i], sizeof(struct PC)); // запись структуры в файл
                row_number++; // увеличение на 1 номера первой отображаемой записи
            }
        }
    ...
    row_number--; // по завершению цикла переменная будет увеличена на 1, поэтому возвращаем действительное значение
    return row_number; // возвращаем номер первой отображаемой записи
}
...
void ViewTable(char *argv[], std::fstream &table_in, int rows_request, int row_number) {
    ...
    *      в виде С-строки. Так как тип уже приводится к int в процессе обработки корректности аргументов, целесообразно
    *      использовать приведенное значение, для избежания дублирования кода)
    *      row_number   - номер первой записи в таблице
    ...
    *******************************************************************************************************************/
    ...
    try {
        ...
        for (int i = 0; i < rows_to_print; i++) {
            table_in.read((char *) &ParkContent[i], sizeof(struct PC)); // чтение данных таблицы в массив структур
        }
        GenerateTablePage(ParkContent,argv,rows_to_print,real_rows,row_number); // вывод основной страницы программы
        ...
        while (mark) { // бесконечный цикл считывания клавиш
            switch (UserKey()) { // считывание кода клавиши введенной пользователем в режиме "реального времени"
                ...
                case Up: {
                    if (row_number > 0) { // если таблица отображена не сначала
                        row_number--;     // уменьшаем номер первой записи
                ...
                case Down: {
                    ...
                    if ((row_number <= rows_to_print - MAX_ROWS_FR) && (rows_to_print > MAX_ROWS_FR)) {
                        row_number++; // увеличиваем номер первой записи
                        /* вывод основной страницы программы */
                        GenerateTablePage(ParkContent,argv,rows_to_print,real_rows,row_number);
                    }
                    break;
...
void GenerateTablePage(struct PC ParkContent[], char* argv[], int rows_to_print, int real_rows, int row_number){
    ...
    *******************************************************************************************************************/
    ...
    PrintTable(ParkContent, rows_to_print, row_number); // Заполнение строк таблицы
